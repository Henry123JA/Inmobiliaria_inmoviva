            ...
            <form method="post" action="{{ route('ciudad.update', $ciudad->id) }}">
                ...
                <div class="shadow overflow-hidden sm:rounded-md">
                    <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <input type="text" name="nombre" id="nombre" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ old('nombre', $ciudad->nombre) }}" required>
                        @error('nombre')
                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <textarea name="descripcion" id="descripcion" class="form-input rounded-md shadow-sm mt-1 block w-full" required>{{ old('descripcion', $ciudad->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
